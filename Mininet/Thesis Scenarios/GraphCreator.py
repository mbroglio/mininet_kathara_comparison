...
timestamp_pattern = re.compile(
    r"^(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}) - CPU and Memory Usage for Mininet-Related Processes"
)
...
def parse_log(log_file_path):
    """Parse a single mininet log file and return the usage data."""
    relative_times = []
    cpu_usage = []
    mem_usage = []
...
def plot_and_save_graphs(relative_times, cpu_usage, mem_usage, folder_name, output_dir):
    ...
    plt.grid(True)
    plt.legend()
    cpu_svg_path = os.path.join(output_dir, f"{folder_name}_mininet_cpu_usage.svg")
    ...
    plt.grid(True)
    plt.legend()
    mem_svg_path = os.path.join(output_dir, f"{folder_name}_mininet_memory_usage.svg")
...
def process_directories(root_dir, output_dir="mininet"):
    """Recursively process all directories starting from root_dir."""
    log_filename = "mininet_usage.log"
    processed_folders = 0
...
if __name__ == "__main__":
    ...
    output_directory = "mininet"
    
    print(f"Searching for mininet_usage.log files in '{root_directory}' and all subdirectories...")
